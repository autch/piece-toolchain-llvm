...
- **Variadic function ABI: last named arg + varargs on stack** — When calling a variadic function, every named argument EXCEPT the last is passed per the normal CC (R12→R15, overflow to stack); the **last named argument and all variadic arguments** go on the stack, contiguously. This is because the SDK's stdarg.h uses `&(lastparm)` to get the stack address, which requires the last named arg + varargs to be in contiguous stack memory — but earlier named args (e.g. `sprintf`'s destination buffer) stay in registers, exactly as the gcc33-built kernel/SDK expect. The earlier "ALL args on the stack" rule was only accidentally correct for `printf` (whose sole named arg IS the last one) and silently corrupted `sprintf`/`fprintf`/etc. This cannot be expressed in TableGen (the CC has no notion of "last named param"), so it is implemented in C++ in `LowerCall` / `LowerFormalArguments`, which split the arg list at the last named parameter and run `CC_S1C33` on the register part. Do NOT implement a "spill register args in callee prologue" pattern like ARM/MIPS — the caller handles everything.
...
- **Struct-by-value args go entirely on stack (§6.5.4)** — For `callee(struct _foo foo, int d)`, only `d` uses R12; `foo` is passed entirely via stack. Structs do NOT consume register slots. **Both** sides must agree: `LowerCall` (caller) excludes byval args from CC analysis and stores struct words directly to the outgoing stack area after the CC-assigned scalar stack args; `LowerFormalArguments` (callee) must mirror this — exclude byval args from CC analysis and hand ISel a FrameIndex pointer into the incoming stack area at the matching offset. The callee side was originally missing: byval pointers were run through the normal CC and expected in R12 while the caller put the struct on the stack, so any function receiving a struct by value (e.g. odemaru's `PDW_DrawBmpEx(DRAW_OBJECT2)`) read garbage. Existing tests only covered the caller side; `byval-struct.ll` now also tests callees.
