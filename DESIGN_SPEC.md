...
| 7 | `fp.lib` | 浮動小数点演算（`__addsf3`, `__divsf3`等） | **compiler-rtで置き換え済み**（`libclang_rt.builtins-s1c33.a`） |
| 8 | `idiv.lib` | 整数除算（`__divsi3`, `__modsi3`等） | **compiler-rtで置き換え済み**（`libclang_rt.builtins-s1c33.a`） |
...
fp.libとidiv.libはcompiler-rtで置き換え済み（Phase 1）。lib.libにはエラッタ（§6参照）があるため、バグのある関数を個別に差し替えるか、newlib等で代替する選択肢もある（Phase 2 作業項目）。
...
[LLVMツールチェイン]
  clang --target=s1c33-none-elf hello.c
    → LLVM backend → ELF .o
  srf2elf boot.o pceapi.lib io.lib lib.lib math.lib string.lib ctype.lib ...
    → ELF .o / .a に変換（fp.lib/idiv.lib は compiler-rt で置き換え済み、変換不要）
  ld.lld -T piece.ld hello.o boot.o \
...
**Phase 1（完了）:** 標準Cヘッダを newlib から提供するよう移行済み。
`sysroot/s1c33-none-elf/include/` の標準ヘッダは newlib サブモジュール（`newlib/`）から生成する。
P/ECE 固有ヘッダ（`pcefunc.h` 等 11 ファイル）は引き続き `sdk/include/` から提供。
関数本体は依然 lib.lib からリンクされる（以下のPhase 2参照）。

**Phase 2（未着手）:** lib.lib の C ライブラリ実装を newlib の C ソースで段階的に置き換える。
- sin()、strtok()、pow()、strtod()、ispunct() の既知バグが解消される
- 完全な置き換えではなく、バグのある関数を個別に優先リンクする方式が現実的

newlib移植に必要なS1C33固有の作業（Phase 1で実施済みのもの含む）:
- `newlib/libc/machine/s1c33/` — ieeefp.h（リトルエンディアン定義）✅
- `newlib/libc/machine/s1c33/setjmp.h` — `_JBLEN=6`, `_JBTYPE=int` ✅
- libgloss syscallスタブ（`_sbrk`, `_write`, `_exit`等）— `tools/crt/crt0.c` で実装済み ✅
- crt0.S とリンカスクリプトは既存のものを流用 ✅
...
**[完了 — compiler-rt Phase 1]** これらはすべて `libclang_rt.builtins-s1c33.a` が提供する。
...
### Post-Phase-6: newlib Phase 2 — 未着手

- lib.lib の libc 実装を newlib C ソースで段階的置き換え（sin/strtok/pow/strtod/ispunct 等のバグ解消）
