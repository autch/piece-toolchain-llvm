...
| Scratch | R4〜R7, R9 | R4〜R5（戻り値兼用） |
| GP | R8(カーネルテーブルベース=0x0) | — |
| 戻り値 | R10, R11 | R4, R5 |
| 引数 | R12〜R15 | R6〜R9 |
...
```
0x000000  内蔵RAM 8KB（システム使用）割込みベクタ、APIテーブル
0x001000  |ユーザ使用可
0x002000  |スタックの底、内蔵RAM末尾
0x100000  外付けSRAM 256KB — アプリヘッダ
0x100020  |アプリコード開始
0x1xxxxx  |BSS末尾
0x1yyyyy  |（アプリヘッダで指定すれば）スタックの底
0x1zzzzz  |ヒープ開始
0x13D000  |システムワーク
0x140000  |(SRAM末尾)
0xC00000  外付けフラッシュ（512KB〜2MB）リセットベクタ＋緊急カーネル
0xC02000  |通常カーネル開始
0xC0C000  |フォント
0xC28000〜 |ファイルシステム
```
...
| R9 | Scratch（caller-saved、R4〜R7と同格） | 一時レジスタ |
...
DataLayout文字列: `e-m:e-p:32:32-i1:8-i8:8-i16:16-i32:32-i64:32-f32:32-f64:32-a:0:32-n32-S32`

> `a:0:32` は gcc33 互換のグローバル変数最低アライメント（4バイト）。SDK描画関数がビットマップデータにワード/ハーフワードアクセスするため必須。
...
- 構造体の値渡しは原則スタック経由。ただし1要素・32ビット以内の構造体はレジスタ渡し（§3.5）。可変引数関数呼び出しは別規約（§3.4）
...
可変引数関数（`printf`、`scanf`等）では、通常の関数と**異なる引数渡し規約**が適用される:

**呼び出し側（caller）:**
- **最後の名前付き引数と可変引数のみ**をスタックに積む。それより前の名前付き引数は通常の CC どおり R12〜R15（あふれた分はスタック）で渡す
- スタックに積まれた引数は `[%sp+0]`、`[%sp+4]`...と連続配置（最後の名前付き引数が先頭）
- 引数領域のサイズ分だけ事前に `sub %sp, N` でスタックを確保し、呼び出し後に `add %sp, N` で復帰
- `printf`（名前付き引数が `fmt` の1個だけ）では「最後の名前付き引数」＝唯一の名前付き引数なので、結果的に全引数がスタックに乗る。`sprintf(buf, fmt, ...)` では `buf` は R12、`fmt` 以降がスタック
...
`va_start`が `&(lastparm)` を取るため、**最後の名前付き引数**がスタック上に存在し、その直後に可変引数が連続している必要がある。それより前の名前付き引数はレジスタにあってもよい（`va_arg` で参照されないため）。`printf` だけを見ると「全引数スタック渡し」に見えるが、それは `printf` の名前付き引数が1個（＝最後の名前付き引数）だからにすぎない。

**LLVM実装への影響:**

TableGen の CC は「最後の名前付き引数」という概念を表現できないため、`CC_S1C33_VarArg` という専用 def は持たず、`S1C33ISelLowering.cpp` の C++ で処理する:

1. **`LowerCall`（呼び出し側）:** calleeがvariadicの場合、引数列を「最後の名前付き引数」で分割し、それより前は `CC_S1C33`（R12〜R15、あふれはスタック）、最後の名前付き引数＋可変引数は `CCAssignToStack` で処理する
2. **`LowerFormalArguments`（callee側）:** isVarArgの場合も同じ分割を行う。callee の `Ins` は名前付き引数のみを含むので、最後の1個をスタックに置く。ARM/MIPSのような「プロローグでレジスタ引数をスタックにスピルしてVarArgsSaveAreaを作る」パターンは**不要**
3. **`LowerVASTART`:** `VarArgsFrameIndex`を、最後の名前付き引数の直後（可変引数領域の先頭）を指すフレームインデックスとして計算
...
**extは拡張対象命令の即値のsignednessを変更しない。** signed即値を受け取る命令に対して使えば符号拡張、unsigned即値を受け取る命令に対して使えばゼロ拡張となる。

**signed即値の命令（sign6 / sign8）:**
- `ld.w %rd, sign6`（レジスタへの即値ロード）
- `and %rd, sign6` / `or %rd, sign6` / `xor %rd, sign6` / `not %rd, sign6`
- `cmp %rd, sign6`
- `jr** sign8`（全条件分岐、`.d`含む） / `call sign8`（`.d`含む） / `jp sign8`（`.d`含む）

**上記以外のすべての即値はunsigned（imm6 / imm10）:**
- `add %rd, imm6` / `sub %rd, imm6`
- `ld.* %rd, [%sp+imm6]` / `ld.* [%sp+imm6], %rs`（SP相対、スケール済み）
- `add %sp, imm10` / `sub %sp, imm10`

**レジスタ間接メモリ `ld.* [%rb]` にextを付けた場合:**
- ディスプレースメントは**unsigned**（ゼロ拡張）
- `ext imm13 / ld.w %rd, [%rb]` → EA = rb + zero_ext(imm13)、範囲は [0, 8191] バイト
- **負のオフセットは不可能。** ext 8190 は -2 ではなく +8190 として解釈される
- gcc33は実際にメモリアクセスに対して負のextオフセットを一切生成しない

**レジスタ間ALU `op %rd, %rs` にextを付けた場合:**
- 3オペランド命令に変化: `%rd = %rs <op> zero_ext(imm13)` （即値はunsigned）
- 2オペランド即値形式 `op %rd, sign6/imm6`（`%rd = %rd <op> imm`）とは異なる
...
ディレイスロットに充填する候補命令の選定規定（マニュアル pp.30 §2.5.12 のホワイトリスト・実機の寛容な実態・本バックエンドの保守的な中間ライン）は `docs/delay-slot-filler.md` に詳述。
...
純正ext33はR9をアドレス計算用スクラッチとして暗黙に使用する（割り込み安全性の問題があるとされていた）。

**初期設計**: R9をReservedとし、レジスタアロケータに割り付けさせない方針だった。ext33のように暗黙のスクラッチとして使わず、通常のレジスタアロケータが管理するレジスタでアドレスをマテリアライズする。

**現在の実装**: R9をR4〜R7と同格のスクラッチ（caller-saved）として扱い、通常のレジスタ割り付け対象に含めている。これが安全な理由:

1. **P/ECEカーネルの割り込みハンドラ**はINT_BEGIN/INT_ENDマクロで `pushn %r15` / `popn %r15` を実行し、R0〜R15すべてを保存・復元する。R9もこの中に含まれるため、割り込みでR9が破壊されることはない。
2. **pceapiスタブ**はR9をcaller-savedスクラッチとして使用している（ext33 ABI慣習）。R9をcaller-savedとして扱うことで、SDK関数呼び出しの前後で矛盾が発生しない。
3. **レジスタ圧の緩和**: R9がReservedだと16本中5本が使用不可（R8, R9, SP, R10-R11も制約あり）となり、複雑な関数でスピルが多発する。R9を開放することでレジスタ圧を大幅に改善できる。

```asm
; 純正ext33（R9を暗黙使用 — 割り込みハンドラが全GPR保存なら安全）
ext symbol@h
ext symbol@m
ld.w %r9, symbol@l
ld.w %r1, [%r9]

; LLVM（レジスタアロケータがR9を含む任意のscratchを選択）
ext symbol@h
ext symbol@m
ld.w %r4, symbol@l   ; R4でもR9でも、アロケータが最適なレジスタを選ぶ
ld.w %r1, [%r4]
```

**注意**: P/ECEカーネル以外の環境（割り込みハンドラがR0〜R15全体を保存しない環境）では、R9をReservedに戻す必要がある。
...
P/ECEにおけるカーネル↔アプリケーション境界は、pcekn.symで定義された固定アドレスの関数ポインタテーブルのみである。カーネルがアプリにmallocされたメモリを渡したり、その逆を要求するAPIは存在しない。したがってCランタイムの内部状態（ヒープ管理、stdioバッファ、errno等）はカーネルとアプリで独立しており、**アプリ側のCランタイムを差し替えてもカーネルとのABI互換性は崩れない**。

ただし以下の2方向の互換性を維持する必要がある:
...
- カーネル側のCランタイムを変更すると、カーネル内部で使用しているlibc関数の動作が変わり、間接的にアプリの動作に影響する可能性がある。
- **カーネルをClangでビルドする場合はCランタイムをSDKのlib.libのまま維持すべき**。srf2elf + sysrootの仕組みにより、コンパイラだけをClangに差し替えてSDKのCランタイムをそのままリンクすることが可能。
...
newlib側の実装内容:
- `libc/include/machine/{ieeefp,setjmp}.h` — リトルエンディアン定義、`_JBLEN=6`, `_JBTYPE=int`
- `libc/machine/s1c33/{setjmp.S, longjmp.S, Makefile.inc}` — S5U1C33000C ABI 準拠（R0-R3 + SP + return-PC を保存）
- `libc/sys/s1c33/{syscalls.c, sbrk.c, _exit.c, write.c, Makefile.inc}` — POSIX syscall スタブ群、newlib's `_sbrk` は `_pceheapstart + _pceheapsize` を base に upward 成長
- `configure.host` 編集（CPU / host / default flags の3箇所）
- `libc/acinclude.m4` — `m4_foreach_w` の MACHINE / SYS_DIR リストに `s1c33` 追加
- `config.sub` — `s1c33` を CPU 認識行に追加
- `configure` / `Makefile.in` — autoreconf 2.69 + automake 1.15.1 で再生成

LLVM backend 側で必要だった追加修正（newlib をビルドする過程で発見）:
- `S1C33ISelLowering.cpp`: 64-bit FP↔I64 変換 libcall 8 個を `setLibcallImpl` で追加登録（`__fixsfdi`, `__fixdfdi`, `__floatdisf`, `__floatdidf`, `__fixunssfdi`, `__fixunsdfdi`, `__floatundisf`, `__floatundidf`）。newlib の `llroundf` 等が float→long long 変換を要求するため
- `S1C33FrameLowering.cpp`: `MFI.getStackSize()` を 4-byte alignment に round up（`MFI.setStackSize(alignTo(...))`）。 nano-vfprintf_float のような大きなフレームで odd-size local が発生したときに alignment assert が発火していた問題を修正

ヒープ配置（`docs/piece-symbols.md` 参照）:
- `_pceheapstart` = `__END_DEFAULT_BSS`（kernel pceHeap zone の基点 = `pceAppHead.bss_end`）
- `_pceheapsize` = 8 KB（kernel pceHeap 用に確保する規約上のサイズ、`--defsym` で調整可）
...
**PC基準**: ext+ext+call/call.d/jp/jp.d のペアでは、3つのリロケーション（REL_H, REL_M, REL_L）は全て分岐命令自身（ext ではなく call/jp）のアドレスを PC 基準とする。各リロケーションが自分自身のアドレスを基準にするのではない。これを間違えるとビットフィールド間のキャリー伝播で分岐先がずれる。

R_S1C33_REL21（ext+call/call.d/jp/jp.d の21ビット版）も同じ基準。
...
### Post-Phase-6: compiler-rt + newlib Phase 1 — **完了**（2026-04）

- **fp.lib / idiv.lib → compiler-rt 置き換え**: `libclang_rt.builtins-s1c33.a` に移行。
  浮動小数点演算・整数除算・64bit整数ランタイムをすべて compiler-rt が提供。
  `compiler-rt/lib/builtins/s1c33/` に S1C33 ABI 準拠アセンブリ実装を追加。
- **64bit ランタイム完全提供**: `__fixsfdi`, `__fixunssfdi`, `__floatdisf`, `__cmpdi2` 等
  エプソン純正未実装だった関数を compiler-rt GENERIC_SOURCES が提供。
- **newlib ヘッダ移行**: 標準C ヘッダを newlib サブモジュール（`autch/newlib-s1c33`）から生成。
  S1C33 機械設定（`_JBLEN=6`, `__IEEE_LITTLE_ENDIAN`, `WIntType=UnsignedInt`）を追加。
  `sysroot` ブートストラップが完全自己完結（`make -C tools/crt` 1コマンドで構築可能）。
- **Clang DriverToolChain 更新**: `-lclang_rt.builtins-s1c33` を自動挿入、`-lfp -lidiv` を除去。
- **cpptest / C++ サポート確認**: `-flto=full` 使用時の `abort()`/`_exit()` LTO 問題を修正
  （`halt.c` をネイティブ ELF でビルド）。`operator new/delete` マングル名を SizeType=UnsignedLong に対応。
- **実機動作確認**: jien.pex / pcecircle.pex / fpkplay.pex すべて正常動作確認（2026-04）。

### Post-Phase-6: newlib Phase 2 Stage A — **完了**（2026-04-28）

- **newlib `libc.a` / `libm.a` のフルビルド + sysroot インストール**: `tools/crt/Makefile` で
  `newlib/newlib/configure` → `make all-target-newlib` → `cp libc.a libm.a $(SYSROOT)/lib/` を自動化。
- **newlib 側 S1C33 ポート完成**: `libc/machine/s1c33/{setjmp.S, longjmp.S}`、
  `libc/sys/s1c33/{syscalls.c, sbrk.c, _exit.c, write.c}`、`configure.host` / `acinclude.m4` 編集、
...
- **frame lowering alignment fix**: `MFI.getStackSize()` を `alignTo(4)` で round up し、
  nano-vfprintf_float のような大きなフレームで発火していた alignment assert を解消。
- **ヒープ設計確定**: `_pceheapstart` / `_pceheapsize` (kernel pceHeap zone) + newlib sbrk above
  (上限チェックなし、kernel work area との衝突は trust モデル) + `_def_vbuff` を SYSERRVBUFF に
  alias して 11 KB BSS / アプリの節約。詳細は `docs/piece-symbols.md`。
- **`_sbrk` 4-byte alignment 保証**: S1C33 は unaligned word/halfword access が trap するため、
  malloc が返すポインタが必ず 4-aligned になるよう sbrk 側で `incr` を round up + 初期 `brk` も
  align-up。
- **iconv パイプライン整備**: clang は `-fexec-charset=` を実装上無視するため、`tools/crt/`
  内で UTF-8 ソース → SJIS 実行バイトの自動変換が必要なファイルは Makefile に
