...
newlibの移植は以下の用途に限定する:

1. **SDK lib.libのバグ関数の差し替え**: sin()、strtok()等を個別にnewlib版で置換。
2. **SDK非依存のアプリケーション開発**: P/ECE SDKに依存しないベアメタルプログラムで完全なCライブラリが必要な場合。

**カーネルビルドにnewlibを使用してはならない**。これはgcc33ビルドの既存.pexとの後方互換性を破壊する恐れがあるため。

newlib移植に必要なS1C33固有の作業:
- `newlib/libc/machine/s1c33/setjmp.S` — R0–R3, SP, 戻りアドレスの保存/復帰（ABI依存）
- autotoolsへの`s1c33-*-elf`トリプル追加
- libgloss syscallスタブ（`_sbrk`, `_write`, `_exit`等）— P/ECEカーネルAPIへのブリッジまたはスタブ返し
- `crt0.S`とリンカスクリプトは既存のものを流用可能
