...
//		2007.09.09	シンボル名の間違いを修正しました。
//				誤「bBUS_CTL_SEBUSST」→正「bBUS_CTL_SBUSST」
//		2015.01.11	「内蔵I/Oメモリ領域にアクセスするコードのサイズ節約」を追加しました。
//
//	NOTE:
...
//{{2015/01/11変更:内蔵I/Oメモリ領域にアクセスするコードのサイズ節約
//#define plc_IO ((volatile struct c_IOtag*)0x40000)
//↓2015/01/11変更:内蔵I/Oメモリ領域にアクセスするコードのサイズ節約
extern volatile struct c_IOtag* const plc_IO; //→c33209.c
//}}2015/01/11変更:内蔵I/Oメモリ領域にアクセスするコードのサイズ節約
...
#define bBUS_CTL_SBUSST		fBUS_CTL.SBUSST		/* External interface method selection */
...
/****************************************************************/
/*	内蔵I/Oメモリ領域にアクセスするコードのサイズ節約	*/
/****************************************************************/

//{{2015/01/11追加:内蔵I/Oメモリ領域にアクセスするコードのサイズ節約
#ifndef plc_IO
...
//#define plc_IO ((volatile struct c_IOtag*)0x30000)	//①ミラーアドレス	┐
#define plc_IO ((volatile struct c_IOtag*)0x40000)	//②正式なアドレス	├どれでも可。即値をロードする場合は①を使用するとコードサイズを削減出来る。例はclippce.cの_precision_pad_isrを参照せよ。
//#define plc_IO ((volatile struct c_IOtag*)0x50000)	//③ミラーアドレス	┘
#endif//plc_IO
//}}2015/01/11追加:内蔵I/Oメモリ領域にアクセスするコードのサイズ節約

//==============================================================================================================================
//■「内蔵I/Oメモリ領域にアクセスするコードのサイズ節約」の使用方法:
// * Sun Jan 11 01:57:35 JST 2015 Naoyuki Sawa
// - 理論上は、無条件で全モジュールに適用しても問題無いはずではあるのだが、影響範囲が大きすぎて検証し切れないので、
//   当面の間は、適用する箇所を明示的に指定して適用する事にした。適用方法は、以下のとおりです。
//
//	□モジュール全体に適用する場合
//	・モジュールの先頭の「#include "clip.h"」のすぐ下に「#undef plc_IO」と書いてください。
//	<例>
//		/* sample.c */
//		#include "clip.h"
//		#undef plc_IO		//「内蔵I/Oメモリ領域にアクセスするコードのサイズ節約」開始	モジュールに適用
//		void func1() {		┐
//			…		│
//		}			│
//		void func2() {		│
//			…		├モジュール内の全ての関数に「内蔵I/Oメモリ領域にアクセスするコードのサイズ節約」が適用されます。
//		}			│
//		void func3() {		│
//			…		│
//		}			┘
//
//	□一つの関数(又は特定の範囲)に適用する場合
//	・関数の前に「#undef plc_IO」,関数の後に「#include "c33209.h"」と書いてください。
//	<例>
//		/* sample.c */
//		#include "clip.h"
//		void func1() {
//			…
//		}
//		#undef plc_IO		//「内蔵I/Oメモリ領域にアクセスするコードのサイズ節約」開始	特定の範囲に適用
//		void func2() {		┐
//			…		├この関数に「内蔵I/Oメモリ領域にアクセスするコードのサイズ節約」が適用されます。
//		}			┘
//		#include "c33209.h"	//「内蔵I/Oメモリ領域にアクセスするコードのサイズ節約」終了	特定の範囲に適用
//		void func3() {
//			…
//		}
//
// - アドレスが'0x40***'の内蔵I/Oメモリへのアクセスが多い範囲に対して適用すると、コードサイズを削減できます。
// - アドレスが'0x48***'の内蔵I/Oメモリへのアクセスが多い範囲に対して適用すると、却ってコードサイズが増える可能性が有ります。
//	＜例＞
//		【適用前】
...
//		Intel915G_WindowsXp_UsbSuspend_ProblemWorkaround_init:
//			xld.w	%r11,[plc_IO]		+3
//			xld.ub	%r10,[%r11+0x2C6]	-1
//			xand	%r10,%r10,0xFC
//			xoor	%r10,%r10,0x01
//			xld.b	[%r11+0x2C6],%r10	-1
//			xbset	[%r11+0x2C8],0		-1
//			xbset	[%r11+0x2C9],0		-1
//			xbclr	[%r11+0x8205],0		±0
//			xld.w	%r10,TBL_IDMA
//			xld.w	[%r11+0x8200],%r10	±0
//			xbset	[%r11+0x8205],0		±0
//			xbset	[%r11+0x8205],0		±0
//			xbset	[%r11+0x294],0		-1
//			xld.w	%r12,100000
//			xcall	pdwait
