...
スタック余裕 = `0x2000 - __firam_end`。配置物のサイズに対してこの余裕が足りない、または再帰的な処理・深いコールスタックが予想される場合は `_stacklen > 0` を検討してください。

実例として odemaru で発生したケース: 同アプリは音楽ライブラリ `muslib`（当時 hot code を `.fastrun` に配置していた。`__firam_end ≈ 0x1bc0`、残り余裕 ≈ 1 KiB）をリンクしていたが、タイトル後のゲーム開始時にファイル展開＋カーネルコールバックの深い呼び出し連鎖で SP が `0x1a38` 付近まで降り、`.fastrun` 上の `MakeWaveSQR_fast` を上書きした。次の音楽処理が壊れた関数を実行してアドレス不整例外になった（前掲の「特定の関数が壊れる／重い入力で壊れる」症状そのもの）。これを受けて **`tools/muslib` は `.fastrun` 配置をやめ、音楽エンジンは通常メモリで動作する**ようになった（gcc33 ビルドと同じ構成）。したがって現在は `-lmuslib` をリンクしても IRAM は消費されない。
